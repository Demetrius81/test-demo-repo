...
export default class MsgFromUserHandler extends Handler {
  private readonly REQUEST_TYPE: RequestMode = RequestMode.msgFromUser;
  ...
  public override handle(request: IRequest): void {
    if (request.type === this.REQUEST_TYPE) {
      if (request.payload && request.payload.messages) {
        ...
        EventGenerator.generateEvent('msgfromuserrecived');
      } else {
