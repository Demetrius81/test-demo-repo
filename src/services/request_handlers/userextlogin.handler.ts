...
export default class UserExtLoginHandler extends Handler {
  private readonly REQUEST_TYPE: RequestMode = RequestMode.userExtLogin;
  ...
  public override handle(request: IRequest): void {
    if (request.type === this.REQUEST_TYPE) {
      if (request.payload && request.payload.user) {
        ...
        EventGenerator.generateEvent('extuserlogin');
      } else {
