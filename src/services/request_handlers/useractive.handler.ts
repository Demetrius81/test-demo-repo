...
export default class UserActiveHandler extends Handler {
  private readonly REQUEST_TYPE: RequestMode = RequestMode.userActive;
  ...
  public override handle(request: IRequest): void {
    if (request.type === this.REQUEST_TYPE) {
      if (request.payload && request.payload.users) {
        ChatStorage.usersOnline = request.payload.users;
      } else {
